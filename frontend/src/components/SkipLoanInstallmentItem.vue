			...
			<div class="flex flex-row items-start gap-3 grow">
				...
				<div class="flex flex-col items-start gap-1.5">
					<div class="text-base font-normal text-gray-800">
						{{ props.doc.active_loans }}
					</div>
					<div class="text-xs font-normal text-gray-500">
						<span>{{`${props.doc.loan_installment}`}}</span>
					</div>
					<div class="text-xs font-normal text-gray-500">
						<span>{{ props.doc.due_amount }}</span>
						<span class="whitespace-pre"> &middot; </span>
						<span>{{ props.doc.posting_date }}</span>
					</div>
				</div>
			</div>
...
const colorMap = {
	"Open": "orange",
	"Approved": "green",
	"Pending at HOD": "orange",
	"Pending at HR" : "orange",
	"Rejected by HOD" : "red",
	"Rejected by HR" : "red",
	"Cancelled" : "red",
}
